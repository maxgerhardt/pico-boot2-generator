...
def main(input_template: pathlib.Path, output_path: pathlib.Path, skus: str = typer.Argument(""), toml_root: str = typer.Argument("../../data/nvm.toml")):
    if "," in skus:
        skus = skus.split(",")
    else:
        skus = [skus]
    skus = ['sku="{}"'.format(f.strip()) for f in skus]
    flashes = cascadetoml.filter_toml(pathlib.Path(toml_root), skus)
    ...
    #for x in flashes["nvm"]:
    #    print("Got flash: " + str(x))
    #print("Found " + str(len(flashes["nvm"])) + " matches")
